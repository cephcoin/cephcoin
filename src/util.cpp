...
const char * const BITCOIN_CONF_FILENAME = "cephcoin.conf";
const char * const BITCOIN_PID_FILENAME = "cephcoind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "cephcoin" is a composite category enabling all CephCoin-related debug output
            if(ptrCategory->count(string("cephcoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\CephCoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\CephCoinCore
    // Mac: ~/Library/Application Support/CephCoinCore
    // Unix: ~/.cephcoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CephCoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/CephCoinCore";
#else
    // Unix
    return pathRet / ".cephcoincore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty cephcoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override cephcoin.conf
