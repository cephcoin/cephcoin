...
pkgdatadir = $(datadir)/cephcoincore
pkgincludedir = $(includedir)/cephcoincore
pkglibdir = $(libdir)/cephcoincore
pkglibexecdir = $(libexecdir)/cephcoincore
...
CONFIG_HEADER = $(top_builddir)/src/config/cephcoin-config.h
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	...
	$(top_srcdir)/src/config/cephcoin-config.h.in \
...
ACLOCAL = ${SHELL} /root/cephcoin/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /root/cephcoin/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /root/cephcoin/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /root/cephcoin/build-aux/missing automake-1.15
AWK = gawk
BDB_CPPFLAGS = 
...
LUPDATE = 
MAINT = 
MAKEINFO = ${SHELL} /root/cephcoin/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
MINIUPNPC_LIBS = 
...
OTOOL = 
OTOOL64 = 
PACKAGE = cephcoincore
PACKAGE_BUGREPORT = https://github.com/cephcoincoin/cephcoin/issues
PACKAGE_NAME = CephCoin Core
PACKAGE_STRING = CephCoin Core 0.12.1
PACKAGE_TARNAME = cephcoincore
PACKAGE_URL = 
...
ZMQ_CFLAGS = 
ZMQ_LIBS = 
abs_builddir = /root/cephcoin
abs_srcdir = /root/cephcoin
abs_top_builddir = /root/cephcoin
abs_top_srcdir = /root/cephcoin
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /root/cephcoin/build-aux/install-sh
...
BITCOIND_BIN = $(top_builddir)/src/cephcoind$(EXEEXT)
BITCOIN_QT_BIN = $(top_builddir)/src/qt/cephcoin-qt$(EXEEXT)
BITCOIN_CLI_BIN = $(top_builddir)/src/cephcoin-cli$(EXEEXT)
BITCOIN_WIN_INSTALLER = $(PACKAGE)-$(PACKAGE_VERSION)-win$(WINDOWS_BITS)-setup$(EXEEXT)
OSX_APP = CephCoin-Qt.app
OSX_DMG = CephCoin-Core.dmg
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  leveldb_baseline.info test_cephcoin_filtered.info total_coverage.info \
  baseline_filtered.info block_test_filtered.info rpc_test.info rpc_test_filtered.info \
  leveldb_baseline_filtered.info test_cephcoin_coverage.info test_cephcoin.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/CephCoin-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/cephcoin-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/cephcoin-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/cephcoin-config.h
$(top_srcdir)/src/config/cephcoin-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/cephcoin-config.h src/config/stamp-h1
...
$(OSX_APP)/Contents/MacOS/CephCoin-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/CephCoin-Qt

$(OSX_DMG): $(APP_DIST_EXTRAS)
	$(GENISOIMAGE) -no-cache-inodes -D -l -probe -V "CephCoin-Core" -no-pad -r -dir-mode 0755 -apple -o $@ dist
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/CephCoin-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_cephcoin.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_cephcoin -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_cephcoin_filtered.info: test_cephcoin.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#block_test.info: test_cephcoin_filtered.info
#	$(MKDIR_P) qa/tmp
...
#rpc_test.info: test_cephcoin_filtered.info
...
#test_cephcoin_coverage.info: baseline_filtered_combined.info test_cephcoin_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_cephcoin_filtered.info -o $@

#total_coverage.info: baseline_filtered_combined.info test_cephcoin_filtered.info block_test_filtered.info rpc_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_cephcoin_filtered.info -a block_test_filtered.info -a rpc_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_cephcoin.coverage/.dirstamp:  test_cephcoin_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_cephcoin.coverage/.dirstamp total.coverage/.dirstamp

# CephCoin: disable the test until upstream switches from convoluted comparison tool
...
clean-local:
	rm -rf coverage_percent.txt test_cephcoin.coverage/ total.coverage/ qa/tmp/ cache/ $(OSX_APP)
